...
class safariotravel_author_widget extends WP_Widget {
...
'safariotravel_author_widget',


// Widget name will appear in UI
esc_html__( '[ safariotravel ] Author Widget', 'safariotravel' ),

// Widget description
array( 'description' => esc_html__( 'Add author content', 'safariotravel' ), )
);
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Author Name', 'safariotravel' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'designation' ) ); ?>"><?php esc_html_e( 'Designation:' ,'safariotravel'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'textarea' ) ); ?>"><?php esc_html_e( 'Short Descriptions:' ,'safariotravel'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Facebook Url' ) ); ?>"><?php esc_html_e( 'Facebook Url:' ,'safariotravel'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Twitter Url' ) ); ?>"><?php esc_html_e( 'Twitter Url:' ,'safariotravel'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Dribbble Url' ) ); ?>"><?php esc_html_e( 'Dribbble Url:' ,'safariotravel'); ?></label>
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'Behance Url' ) ); ?>"><?php esc_html_e( 'Behance Url:' ,'safariotravel'); ?></label>
...
<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>"><?php _e( 'Image', 'safariotravel' ); ?>:</label>
	<div class="safariotravel-media-container">
		<div class="safariotravel-media-inner">
			...
			<span class="safariotravel-no-image" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-noimg" <?php echo wp_kses_post( $no_img_style ); ?>><?php _e( 'No image selected', 'safariotravel' ); ?></span>
		</div>
	
	<input type="text" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'image' ) ); ?>" value="<?php echo esc_attr( $image ); ?>" class="safariotravel-media-url" />

	<input type="button" value="<?php echo _e( 'Remove', 'safariotravel' ); ?>" class="button safariotravel-media-remove" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-remove" <?php echo wp_kses_post( $img_style ); ?> />

	<?php $button_text = ( $image != '' ) ? __( 'Change Image', 'safariotravel' ) : __( 'Select Image', 'safariotravel' ); ?>
	<input type="button" value="<?php echo esc_html( $button_text ); ?>" class="button safariotravel-media-upload" id="<?php echo esc_attr( $this->get_field_id( 'image' ) ); ?>-button" />
	<br class="clear">
	</div>
</p>
<style>
.safariotravel-media-container {
	width: 98%;
}

.safariotravel-media-inner {
...
.safariotravel-media-inner img {
	max-width: 100%;
	height: auto;
}

.safariotravel-media-url {
	display: none;
}

.safariotravel-media-remove {
	float: left;
	width: 48%;
}

.safariotravel-media-upload {
	float: right;
	width: 48%;
}
</style>
...
jQuery(function($){
    ...
    wpshed_image_upload( '.safariotravel-media-upload' );
    ...
    wpshed_image_remove( '.safariotravel-media-remove' );
...
} // Class safariotravel_author_widget ends here


// Register and load the widget
function safariotravel_author_load_widget() {
	register_widget( 'safariotravel_author_widget' );
}
add_action( 'widgets_init', 'safariotravel_author_load_widget' );