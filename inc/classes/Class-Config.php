...
// Final Class
final class safariotravel {

	// Theme Version
	private $safariotravel_version = '1.0';
	...
	private function setup() {
		...
		$enqueu = new safariotravel_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->safariotravel_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'safariotravel_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'safariotravel', SAFARIOTRAVEL_DIR_PATH . '/languages' );
		...
		add_image_size( 'safariotravel_widget_post_thumb', 80, 80, true ); 

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'safariotravel-np-thumb', 60, 60, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'safariotravel' ),
            'social-menu'   => esc_html__( 'Social Menu', 'safariotravel' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = SAFARIOTRAVEL_DIR_CSS_URI;
		$jsPath  = SAFARIOTRAVEL_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					'handler'		=> 'safariotravel-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->safariotravel_version,
				),
				array(
					'handler'		=> 'safariotravel-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'safariotravel' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate safariotravel theme customizer
		$safariotravel_theme_customizer = new safariotravel_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'safariotravel_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'safariotravel_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'safariotravel-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'safariotravel_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'safariotravel-elementor-default-style',  SAFARIOTRAVEL_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->safariotravel_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'safariotravel_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'safariotravel-elementor-notice', SAFARIOTRAVEL_DIR_JS_URI.'safariotravel-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'safariotravel-elementor-notice',
				'safariotravelElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'safariotravel-elementor-notice-nonce' ),
				)
			);
		}
